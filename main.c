...
//#include "./decimate_test_sin_48k.txt" // yes
...
// *************************8 decimate by 8 with halfband IIR allpass decompositions ****

void decimate_8x_iirHB(
	q31_t * pSrc,
	q31_t * pDst,
	uint32_t len)
{
	uint32_t k,j1=0;
	static q31_t ap_state_stage0_zm1=0; // 1st number os decimation stage
	static q31_t ap_state_stage0_zm0=0;

	static q31_t ap_state_stage1_zm1=0; // 1st number os decimation stage
	static q31_t ap_state_stage1_zm0=0;

	static q31_t ap_state_stage2_upper_zm2=0;
	static q31_t ap_state_stage2_upper_zm1=0;
	static q31_t ap_state_stage2_upper_zm0=0;

	static q31_t ap_state_stage2_lower_zm1=0;
	static q31_t ap_state_stage2_lower_zm0=0;

	static q31_t deci_stage0_out0 = 0;
	static q31_t deci_stage0_out1 = 0;
	static q31_t deci_stage0_out2 = 0;
	static q31_t deci_stage0_out3 = 0;

	static q31_t deci_stage1_out0 = 0;
	static q31_t deci_stage1_out1 = 0;


	static q31_t coeff_d2_n0_upper = 0x0E4C2F83;
	static q31_t coeff_d1_n1_upper = 0x78C7E282;
	static q31_t coeff_d1_n1_lower = 0x39DE69AD;
	static q31_t mult_temp1;
	static q31_t mult_temp2;
	static q31_t allpass_upper,allpass_lower;



	k = len;
	//j0 = 1; // allpass starts with odd inputs and skips by 2
	j1 = 0; // output pointer into array
	...
	while(k > 0) { // each loop produces 2 outputs at 2X rate, so use DMALEN/8
			...
			ap_state_stage0_zm0 = (*pSrc-- >> 1) - (ap_state_stage0_zm1 >> 2) - (ap_state_stage0_zm1 >> 3); // scale input by 1/2 so the output does not need to be scaled by 1/2
			//ap_state_stage0_zm0 = (dmaDestBuff_32bit[j0] >> 1) - (ap_state_stage0_zm1 >> 2) - (ap_state_stage0_zm1 >> 3); // scale input by 1/2 so the output does not need to be scaled by 1/2
//			deci_stage0_out0 = ap_state_stage0_zm1 + (ap_state_stage0_zm0 >> 2) + (ap_state_stage0_zm0 >> 3) + (dmaDestBuff_32bit[j0-1] >> 1);
			deci_stage0_out0 = ap_state_stage0_zm1 + (ap_state_stage0_zm0 >> 2) + (ap_state_stage0_zm0 >> 3) + (*pSrc >> 1);
			pSrc+=3;
			...
			ap_state_stage0_zm1 = ap_state_stage0_zm0;
			ap_state_stage0_zm0 = (*pSrc-- >> 1) - (ap_state_stage0_zm1 >> 2) - (ap_state_stage0_zm1 >> 3); // scale input by 1/2 so the output does not need to be scaled by 1/2
			deci_stage0_out1 = ap_state_stage0_zm1 + (ap_state_stage0_zm0 >> 2) + (ap_state_stage0_zm0 >> 3) + (*pSrc >> 1);
			pSrc+=3;
			...
			ap_state_stage0_zm1 = ap_state_stage0_zm0;
			ap_state_stage0_zm0 = (*pSrc-- >> 1) - (ap_state_stage0_zm1 >> 2) - (ap_state_stage0_zm1 >> 3); // scale input by 1/2 so the output does not need to be scaled by 1/2
			deci_stage0_out2 = ap_state_stage0_zm1 + (ap_state_stage0_zm0 >> 2) + (ap_state_stage0_zm0 >> 3) + (*pSrc >> 1);
			pSrc+=3;
			...
			ap_state_stage0_zm1 = ap_state_stage0_zm0;
			ap_state_stage0_zm0 = (*pSrc-- >> 1) - (ap_state_stage0_zm1 >> 2) - (ap_state_stage0_zm1 >> 3); // scale input by 1/2 so the output does not need to be scaled by 1/2
			deci_stage0_out3 = ap_state_stage0_zm1 + (ap_state_stage0_zm0 >> 2) + (ap_state_stage0_zm0 >> 3) + (*pSrc >> 1);
			pSrc+=3;
			...
			ap_state_stage1_zm0 = (deci_stage0_out1 >> 1) - (ap_state_stage1_zm1 >> 2) - (ap_state_stage1_zm1 >> 3); // scale input by 1/2 so the output does not need to be scaled by 1/2
			//deci_stage1_out_left[j1++] = ap_state_stage1_zm1 + (ap_state_stage1_zm0 >> 2) + (ap_state_stage1_zm0 >> 3) + (deci_stage0_out0 >> 1);
			deci_stage1_out0 = ap_state_stage1_zm1 + (ap_state_stage1_zm0 >> 2) + (ap_state_stage1_zm0 >> 3) + (deci_stage0_out0 >> 1);

			ap_state_stage1_zm1 = ap_state_stage1_zm0;
			ap_state_stage1_zm0 = (deci_stage0_out3 >> 1) - (ap_state_stage1_zm1 >> 2) - (ap_state_stage1_zm1 >> 3); // scale input by 1/2 so the output does not need to be scaled by 1/2
			//	deci_stage1_out_left[j1++] = ap_state_stage1_zm1 + (ap_state_stage1_zm0 >> 2) + (ap_state_stage1_zm0 >> 3) + (deci_stage0_out2 >> 1);
			deci_stage1_out1 = ap_state_stage1_zm1 + (ap_state_stage1_zm0 >> 2) + (ap_state_stage1_zm0 >> 3) + (deci_stage0_out2 >> 1);

			// at this point I've processed 8 inputs from the dma array and produced 2 outputs
			// now for the final stage, which has a 2nd-order allpass in the un-delayed branch and a 1st-order allpass in the delayed branch

				// 2nd order allpass
			ap_state_stage2_upper_zm2 = ap_state_stage2_upper_zm1; // non-delayed branch
			ap_state_stage2_upper_zm1 = ap_state_stage2_upper_zm0;
			mult_temp1 = (q31_t)  ( ((q63_t) ap_state_stage2_upper_zm1 * coeff_d1_n1_upper) >> 32);
			mult_temp2 = (q31_t)  ( ((q63_t) ap_state_stage2_upper_zm2 * coeff_d2_n0_upper) >> 32);
			ap_state_stage2_upper_zm0 = (deci_stage1_out1 >> 1) - mult_temp1 - mult_temp2;
			mult_temp1 = (q31_t)  ( ((q63_t) ap_state_stage2_upper_zm0 * coeff_d2_n0_upper) >> 32);
			mult_temp2 = (q31_t)  ( ((q63_t) ap_state_stage2_upper_zm1 * coeff_d1_n1_upper) >> 32);
		    allpass_upper = mult_temp1 + mult_temp2 + ap_state_stage2_upper_zm2;

		    	// 1st order allpass for the delayed branch
			ap_state_stage2_lower_zm1 = ap_state_stage2_lower_zm0;
			mult_temp1 = (q31_t)  ( ((q63_t) ap_state_stage2_lower_zm1 * coeff_d1_n1_lower) >> 32);
			ap_state_stage2_lower_zm0 = (deci_stage1_out0 >> 1) - mult_temp1;
			mult_temp1 = (q31_t)  ( ((q63_t) ap_state_stage2_lower_zm0 * coeff_d1_n1_lower) >> 32);
			allpass_lower = mult_temp1 + ap_state_stage2_lower_zm1;

			*pDst++ =  allpass_lower+allpass_upper;
			debugBuff[j1++] = allpass_lower+allpass_upper;
...
void DMA0_IRQHandler()
{
	...
	switch(magpie_FS) { // do the correct filter for each sample-rate
		...
		case fs_96k_1ch: // timing test 6.7ms
			...
			//data_converters_i24_to_q31(dmaDestBuff, dmaDestBuff_32bit, DMA_buffLen_bytes);
			arm_fir_decimate_fast_q31_1ch(2,deci_96k_numcoeffs_0,firCoeffs_96k_0,firState_stage0_left,dmaDestBuff_32bit,deci_stage0_out_left,DMA_buffLen);
			arm_fir_decimate_fast_q31_HB_1ch(deci_96k_numcoeffs_1,firCoeffs_96k_1,firState_stage1_left,deci_stage0_out_left,deci_stage1_out_left,buffLen_deci2x);
