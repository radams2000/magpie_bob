...
// Right channel
  //q31_t *pStateRight = S->pStateRight;                     /* State pointer */
  q31_t *pStateRight = pStateInitRight;
  ...
  q31_t *pxRight;                                     /* Temporary pointers for state buffer, not used */                                  /* Temporary pointers for coefficient buffer */
  q31_t sum0Right;                                    /* Accumulator */
  q31_t x1Right;
  q31_t acc0Right, acc1Right;
  q31_t *px0Right, *px1Right;
...
//************* // copy 4 sample of input buff into state buff offset by numTaps-1 ******/
    do
    {
      ...
      *pStateRightCurnt++ = *pSrcRight++; // pStateLeft gets incremented by 4 every big loop
    ...
    px0Left = pStateLeft;
    px0Right = pStateRight;

    //px1Left = pStateLeft + S->M;
    px1Left = pStateLeft + 2; // HB, only 2 allowed
    px1Right = pStateRight + 2; // HB, only 2 allowed
...
// **** haffband edit, add x * center_coeff here , coeff is assumed to be 0.5
    ...
    x0Right = *(pStateRight  + centerTapIndex); // data aligned with center tap for 1st sample
    x1Right = *(pStateRight + 2 + centerTapIndex); // data aligned with centertap for 2nd sample
    acc0Right = (q31_t) acc0Right + (x0Right >> 2);
    acc1Right = (q31_t) acc1Right + (x1Right >> 2);
    ...
    pStateRight = pStateRight + 4; // HB M=2
